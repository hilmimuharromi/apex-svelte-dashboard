<script lang="ts">
	...
	type Props = { data: Point[]; height?: number; class?: string; fill?: string; stroke?: string };
	let { data, height = 240, class: cls, fill = 'var(--color-primary)', stroke = 'var(--color-primary)' }: Props = $props();
	...
	const areaPath = $derived(`${path} L ${w - pad} ${h - pad} L ${pad} ${h - pad} Z`);
</script>
